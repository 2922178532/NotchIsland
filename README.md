...
- 悬停卡片约 0.3 秒即可预览文本、放大的图片或文档内容；不支持内容预览的文件显示图标
...
| 从卡片往外拖 | 把该文件复制到目标应用 |
| 悬停卡片 | 约 0.3 秒后显示文本预览、等比例大图或文档内容缩略图；不支持预览的文件显示图标，双击可打开完整内容 |
...
- **空闲时显示**：鼠标不在岛上时的状态，可选「收起（原生刘海）」或「悬停（常显功耗）」，默认收起。选悬停后岛会一直略微放大并常显实时功耗，代价是刘海区域始终比原生大一圈。
